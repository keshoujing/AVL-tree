...
class AVL_Tree
{
    struct Node
    {
        ...
        Node* right;
        int balanceFactor;
        Node(string ID, string name) : gatorID(ID), left(nullptr), right(nullptr), balanceFactor(0), name(name){}
    };
    Node* root;
    //private functions
    Node* insertFunction(string ID, string name, Node* rootNode);
    ...
    Node* searchNode(Node* node, string ID);
...
//insert node.
AVL_Tree::Node * AVL_Tree::insertFunction(string ID, string name, Node* rootNode) {
    if (!rootNode)
    {
    ...
    if (rootNode->gatorID.compare(ID) < 0) //base on project instruction, there will be no equal situation.
    {
...
void AVL_Tree::insert(string ID, string name) {
...
void AVL_Tree::searchID(string ID) {
...
AVL_Tree::Node *AVL_Tree::searchNode(AVL_Tree::Node *node, string ID) {
    if (!node)
        return nullptr;
    if (node->gatorID.compare(ID) == 0)
        return node;

    node = node->gatorID.compare(ID) > 0 ? searchNode(node->left, ID) : searchNode(node->right,ID);
...
AVL_Tree::Node *AVL_Tree::remove(AVL_Tree::Node* node, string ID) {
    if (!node)
        return nullptr;

    if (stoi(node->gatorID) > stoi(ID))
    {
...
void commandProcess(AVL_Tree &AVL)
{
    ...
    for (int i = 0; i < firstInput; ++i) {
        ...
        if (input == "insert")
        {
            cin.sync();
            getline(cin, input);
            ID = input.substr(input.length() - 8, input.length() - 1); //pick all numbers out, only allow 8 numbers.
